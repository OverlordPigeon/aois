...
def delete_excess_implicants(expr, is_sknf):
    ...
    for i in range(len(expr_list)):
        ...
        var_vocabulary = get_var_vocabulary(temp, target)
        equals = execute(expr_list, var_vocabulary, ch_1, is_sknf)
        excesses[i] = 1
...
def execute(expr, var_vocabulary, ch, is_sknf):
    vars = get_vars("".join(expr))
    additional_vars = []
    for var in vars:
        if var not in var_vocabulary:
            var_vocabulary[var] = "0"
            additional_vars.append(var)
    if len(additional_vars) > 1:
        return False
    first_result = laba_2.execute_logical_statement(
        laba_2.get_postfix(ch.join(expr)), var_vocabulary
    )
    for var in additional_vars:
        var_vocabulary[var] = "1"
...
def get_var_vocabulary(expr, table):
    var_vocabulary = {}
    vars = get_vars(expr)
    for i, var in enumerate(vars):
        var_vocabulary[var] = table[i]
    return var_vocabulary
